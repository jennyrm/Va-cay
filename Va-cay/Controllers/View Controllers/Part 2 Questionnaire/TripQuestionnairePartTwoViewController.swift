...
class TripQuestionnairePartTwoViewController: UIViewController {
    ...
    func createLabelCalendarButton(with flightLabel: UILabel) -> UIStackView {
        ...
        flightLabel.backgroundColor = Colors.textfieldBackgroundColor
    ...
    func setupConstraints() {
        ...
        nextButton.topAnchor.constraint(equalTo: scrollView.bottomAnchor, constant: 40).isActive = true
        ...
        nextButton.bottomAnchor.constraint(equalTo: safeArea.bottomAnchor, constant: -48).isActive = true
    }
...
extension TripQuestionnairePartTwoViewController: UITextFieldDelegate {
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        guard let text = textField.text else { return false }
        let newString = (text as NSString).replacingCharacters(in: range, with: string)
        textField.text = format(with: "$XXXXXXX", phone: newString)
        return false
    }
